...
'''Handposition aus Gazebo auslesen'''
def get_hand_position():
    msg = rospy.wait_for_message('/gazebo/link_states', LinkStates)
    hand_position_x = (msg.pose[9].position.x + msg.pose[10].position.x) / 2 #Mittelwert der 2 Gripper Finger
...
def reset(msg):
    joint_reset = [0, -0.4, 0, -1.17, 0, 0.785, math.pi / 4, 0.00, 0.00]
...
rospy.init_node('reset_service', anonymous=False)
s = rospy.Service('reset_env', ResetJoints, reset)
print("reset_node aktiv")
rospy.spin()
