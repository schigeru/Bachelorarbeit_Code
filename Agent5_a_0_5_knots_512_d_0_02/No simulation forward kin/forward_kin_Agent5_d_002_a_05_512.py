...
'''"Simuliert" Panda Gelenkveränderungen rein in Panda ohne Collision/SelfCollision Awareness - kann genutzt werden um
RL-Modelle zu trainieren, bei denen SelfCollision keine Rolle spielt, da zwischen Start und Ziel keine Gefahr für Kollisionen besteht'''
...
class PandaRobotGymEnv(gym.Env):
    ...
    '''Hilfsmethode zur Berechnung der Transmatrix für Forward Kinematics'''
    def get_t_n(self, sin_theta, cos_theta, n):
        ...
        return t_n
    '''Berechnung der Transmatrix für Forward Kinematics'''
    def get_trans_matrix(self):
        ...
        return trans_matrix
    '''Berechnung der Abweichung Endeffektor Orientierung vom Ziel in Quaternions'''
    ...
    '''Step Methode des Environment, Skalierung der Aktion hat zu besserem Lernverhalten geführt'''
    def step(self, action):
        ...
        trans_matrix = self.get_trans_matrix()
        hand_pos = trans_matrix[0:3, 3] #Endeffektor x,y,z ergibt sich aus oberen 3 Werten der letzte Transmatrixspalte
        z = hand_pos[2] - self.goal[2] #Distanz in z
        goal_dist = np.linalg.norm(self.goal - hand_pos) #Distanz Endeffektor zum Ziel
        vector_dist = self.goal - hand_pos #Distanz in x,y,z
        obs = self.joint_state
        obs = np.append(obs, vector_dist)

        q = tf.quaternion_from_matrix(trans_matrix) #Berechnet Quat aus TransMatrix
        quaternion_dist = self.calc_quaternion_norm(q) #Berechnet Orientierungsunterschied zur Zielorientierung
        ...
        if self.joint1 < self.joint1_threshold_min or self.joint1 > self.joint1_threshold_max \
            ...
            elif goal_dist > 0.08 and z < 0.05 or z < 0: #fördert anfahren von oben
                reward = 5 * (-goal_dist - 0.5 * quaternion_dist)
                self._env_step_counter += 1
                return obs, reward, self.done, {}

            else:
                reward = -goal_dist - 0.5 * quaternion_dist
                #print("Action: ", action)
                #print("Handpos: ", hand_pos)
                #print("Goal: ", self.goal)
                #print("Quaternion", q)
